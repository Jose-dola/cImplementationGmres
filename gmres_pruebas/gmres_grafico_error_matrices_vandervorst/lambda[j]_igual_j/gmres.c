...
double* gmresUnlimited( int dim, double* b, void (*A_product)(double*,int,double*), double rc, double tol )
{
  ...
  */
  ...
  while (1)
  {
    ...
    /* print the error(j)/error(j-1) */
    fprintf(stderr,"%d  % 10le\n", j,sine[j]);
...
/* 
 * RESTARTED GMRES algorithm to solve Ax=b
 *
 * PARAM:
 *   m:         restart parameter
 *   dim:       dimension of the system
...
double* gmresM( int m, int dim, double* b, void (*A_product)(double*,int,double*), double rc, double tol, double** error )
{
  ...
  /* set number of restarted loops, iterations, number of flops, number of square roots and number of Av products */
  restarted_loops = 1;
  iterations      = 1;
  flops           = 0;
  sroots          = 0;
  ...
  /* bLS[0] = ||b|| */
  bLS[0] = 0;
  for (i=0; i<dim; i++) { bLS[0]+=b[i]*b[i]; flops=flops+2; }
  bLS[0] = sqrt(bLS[0]); sroots++;
  ...
  while( bLS[0] > tol ) 
  {
    ...
    /* Q[0] = err/||err|| */
    for (i=0; i<dim; i++) {Q[0][i]=err[i]/bLS[0]; flops++;}
    ...
    for(j=0; j<m; j++)
    {
      /***************** ARNOLDI ITERATION ********************/
      /********************************************************/
      A_product(Q[j],dim,Q[j+1]); AvProducts++;
      /* save ||Q[j+1]|| for the refinement check */
      aux = 0;
      for (k=0; k<dim; k++) {aux+=Q[j+1][k]*Q[j+1][k]; flops=flops+2;}
      aux = sqrt(aux); sroots++;
      ...
      for (k=0; k<j; k++)
      {
        /* product of Q[k]*Q[j+1] */
        H[j][k] = 0;
        for (i=0; i<dim; i++) {H[j][k]+=Q[k][i]*Q[j+1][i]; flops=flops+2;}
        /* Q[j+1] = Q[j+1] - H[j][k]*Q[k] */
        for (i=0; i<dim; i++) {Q[j+1][i]=Q[j+1][i]-H[j][k]*Q[k][i]; flops=flops+2;}
      }
      /* last iteration of the previous "for" with k=j (and code changes) to calculate H[j][j] and H[j][j+1]=||Q[j+1]|| */
      /* product of Q[j]*Q[j+1] */
      H[j][j] = 0;
      for (i=0; i<dim; i++) {H[j][j]+=Q[j][i]*Q[j+1][i];flops++;}
      /* Q[j+1] = Q[j+1] - H[j][i]*Q[i]  and  H[j][j+1] = ||Q[j+1]|| */
      H[j][j+1] = 0;
      for (i=0; i<dim; i++) { Q[j+1][i]=Q[j+1][i]-H[j][j]*Q[j][i]; H[j][j+1]+=Q[j+1][i]*Q[j+1][i]; flops=flops+4;}
      H[j][j+1] = sqrt(H[j][j+1]); sroots++;
      ...
      if ( H[j][j+1]/aux <= rc)
      {
        for (k=0; k<j; k++)
        {
          /* aux = Q[k]*Q[j+1] */
          aux = 0;
          for (i=0; i<dim; i++) {aux+=Q[k][i]*Q[j+1][i];flops=flops+2;}
          /* Q[j+1] = Q[j+1] - aux*Q[k] */
          for (i=0; i<dim; i++) {Q[j+1][i]=Q[j+1][i]-aux*Q[k][i];flops=flops+2;}
          H[j][k] = H[j][k] + aux; flops++;
        }
        /* last iteration of the previous "for" with k=j (and code changes) to calculate H[j][j] and H[j][j+1]=||Q[j+1]|| */
        /* aux = Q[j]*Q[j+1] */
        aux = 0;
        for (i=0; i<dim; i++) {aux+=Q[j][i]*Q[j+1][i];flops++;}
        /* Q[j+1] = Q[j+1] - aux*Q[j] and get ||Q[j+1]|| */
        H[j][j+1] = 0;
        for (i=0; i<dim; i++) { Q[j+1][i]=Q[j+1][i]-aux*Q[j][i]; H[j][j+1]+=Q[j+1][i]*Q[j+1][i]; flops=flops+4;}
        H[j][j] = H[j][j] + aux; flops++;
        /* H[j][j+1] = ||Q[j+1]|| */ 
        H[j][j+1] = sqrt(H[j][j+1]); sroots++;
      }
      ...
      if ( fabs(H[j][j+1]) < tol ) 
      {
        ...
        {
          aux       = R[j][k];
          R[j][k]   = aux*cosine[k] + H[j][k+1]*sine[k]; flops=flops+3;
          R[j][k+1] = H[j][k+1]*cosine[k] - aux*sine[k]; flops=flops+3;
        }
        mLS     = j;
        break; 
      }
      ...
      /* normalize Q[j+1] */
      for (i=0; i<dim; i++) {Q[j+1][i] = Q[j+1][i]/H[j][j+1];flops++;}
      ...
      /* compute and apply Givens rotation to put 0 in the subdiagonal */
      if ( fabs(H[j][j+1]) > fabs(R[j][j]) ) { aux=R[j][j]/H[j][j+1]; sine[j]=1./sqrt(1+aux*aux); cosine[j]=sine[j]*aux;  flops=flops+5; sroots++;}
      else                                   { aux=H[j][j+1]/R[j][j]; cosine[j]=1./sqrt(1+aux*aux); sine[j]=cosine[j]*aux; flops=flops+5; sroots++;}
      /* apply the rotation found (remember that bLS[i+1]=0) */
      R[j][j]  = R[j][j]*cosine[j] + H[j][j+1]*sine[j]; flops=flops+3;
      bLS[j+1] = -bLS[j]*sine[j]; flops++;
      bLS[j]   = bLS[j]*cosine[j]; flops++;

     /*** SHOW THE ERROR ***/
     printf("||r(%d)(%d)|| = % 10le\n",restarted_loops,iterations,fabs(bLS[j+1]));
     iterations++;

    }/***************** END m ITERATIONS OF GMRES ****************/
    
    /*** actualize error showing parameters ***/
    restarted_loops++;
    iterations=1;
    ...
    {
      y[k] = bLS[k];
      for (i=k+1; i<=mLS; i++) {y[k]-=y[i]*R[i][k];flops=flops+2;}
      y[k] = y[k]/R[k][k]; flops++;
    } 

    /* compute next aproximation of the solution */
    for (k=0; k<dim; k++)
      for (i=0; i<=mLS; i++) {x[k]+=Q[i][k]*y[i];flops=flops+2;}

    /* compute the error and prepare the next iteration */
    A_product(x,dim,err); AvProducts++;
    for(i=0; i<dim; i++) {err[i]=b[i]-err[i];flops++;}

    /****** PREPARING THE NEXT ITERATION *******/
    /* Q[0] = err/||err|| and bLS[0] = ||err|| */
    bLS[0] = 0;
    for (i=0; i<dim; i++) {bLS[0]+=err[i]*err[i];flops=flops+2;}
    bLS[0] = sqrt(bLS[0]); sroots++;
  ...
  /*** SHOW NUMBER OF FLOPS AND SQUARE ROOTS DONE ***/
  printf("m=%d:    squareRoots=%d    GMRESflops=%d    AvProducts=%d    flops(GMRES+AvProducts)=%d\n",m,sroots,flops,AvProducts,dim*dim*2*AvProducts+flops);
