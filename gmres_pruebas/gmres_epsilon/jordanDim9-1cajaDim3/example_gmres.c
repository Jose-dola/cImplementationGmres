...
*/
  /* GMRESm 'm' epsilon text */
    ...
    puts("");
    for (epsilon=1e-9; epsilon < 1e-1+1e-2; epsilon*=10)
    {
      for (i=1; i<=dim; i++)
      {
        printf("%le %d ",epsilon,i);
...
void A_product(double* v, int dim, double* Av)
{
  ...
  /* D product (D Jordan diagonal 111222333 con caja de jordan de dimension 3 en el principio de la diagonal) */
  Av[0] = Av[0]*(D[0]-epsilon) + Av[1];
  Av[1] = Av[1] + Av[2];
  for (i=3; i<dim; i+=3) Av[i] = Av[i]*(D[i]-epsilon);
  for (i=4; i<dim; i+=3) Av[i] = Av[i]*D[i];
