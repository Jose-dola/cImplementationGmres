...
int main(void)
{
  ...
  for (i=0; i<dim-1; i++) b[i]=A[i][i]+A[i+1][i+1];
  b[dim-1]=A[dim-1][dim-1]+A[0][0];
...
/*    epsilon=-0.5;
    ...
    puts("");
*/
    for (epsilon=-0.1; epsilon < 0.1+1e-5; epsilon+=0.02)
    {
/*      showAperturbed(dim);*/
      for (i=1; i<=dim; i++)
      {
...
void showAperturbed(int dim)
{
  ...
  for (i=0; i<dim; i++)
  {
    ...
    for (j=2; j<4; j++) printf("% 10lf ",A[i][j]);
    puts("");
  }
}
