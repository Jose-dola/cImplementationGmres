...
double* gmresUnlimited( int dim, double* b, void (*A_product)(double*,int,double*), double rc, double tol )
{
  ...
  double** H;      /* arnoldi's H (and R of QR givens) */
  ...
  /* sine and cosine for the QR givens */
  ...
  */
  ...
  while (1)
  {
    ...
    {
      aux       = H[j][k];
      H[j][k]   = aux*cosine[k] + H[j][k+1]*sine[k];
      H[j][k+1] = H[j][k+1]*cosine[k] - aux*sine[k];
    }

    /* compute and apply Givens rotation to put 0 in the subdiagonal */
    if ( fabs(H[j][j+1]) > fabs(H[j][j]) ) { aux=H[j][j]/H[j][j+1]; sine[j]=1./sqrt(1+aux*aux); cosine[j]=sine[j]*aux;  }
    else                                   { aux=H[j][j+1]/H[j][j]; cosine[j]=1./sqrt(1+aux*aux); sine[j]=cosine[j]*aux; }
    /* apply the rotation found (remember that bLS[i+1]=0) */
    H[j][j]  = H[j][j]*cosine[j] + H[j][j+1]*sine[j];
  ...
  /*** FREE MEMORY ***/
  for(i=0; i<=j; i++) { free(H[i]); free(Q[i]); }
  free(Q);
  free(H);
  free(sine);
  free(cosine);
 ...
 *   rc:        refinement condition of arnoldi (for example 0.25)
 *   tol:       error tolerance
 *   error:     we put in this pointer the error vector of the solution returned (it is not necessary to allocate previously)
 *              we ignore if the input is NULL
 *   x:         we put in this pointer the best approximation of the solution that we have found 
 *              (it is necessary to allocate previously)
 *
 * RETURN:
 *     0  if we have found a solution with the 'tol' tolerance
 *     -1 if we have not found a solution with the 'tol' tolerance
*/
int gmresM( int m, int dim, double* b, void (*A_product)(double*,int,double*), double rc, double tol, double** error , double* x)
{
  ...
  double** Q;       /* arnoldi's Q */
  double** H;       /* arnoldi's H (and R of QR givens) */
  double   previousbNorm; /* to the stagnation check */
  ...
  /* sine and cosine for the QR givens and least square problem */
  ...
  while( bLS[0] > tol ) 
  {
    ...
    {
      y[k] = bLS[k];
      for (i=k+1; i<=mLS; i++) y[k]-=y[i]*H[i][k];
      y[k] = y[k]/H[k][k];
    } 
...
for(i=0; i<dim; i++) printf("%.20le ",err[i]);
puts("");

    /****** STAGNATION CONDITION ******/
    if( fabs( (bLS[0]-previousbNorm)/previousbNorm ) < TOL )
    {
      ...
      /*** FREE MEMORY ***/
      for(i=0; i<m; i++) { free(H[i]); free(Q[i]); }
      free(Q[m]);
      free(Q);
      free(H);
      free(sine);
      ...
      /*** we have not obtained the wanted precision ***/
      return -1;
    } 
  ...
  /*** we have get the wanted precision ***/
  return 0;
}
